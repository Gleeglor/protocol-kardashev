...
pub fn main() !void {
    ...
    // ── Shader setup ────────────────────────────────────────────────
    const vertex_shader: []u8 = try read_file("res/shaders/in-world.vert");
    const fragment_shader: []u8 = try read_file("res/shaders/in-world.frag");
    ...
    while (!window.shouldClose()) {
        ...
        model = zmath.mul(model, zmath.rotationZ(@floatCast(glfw.getTime())));
